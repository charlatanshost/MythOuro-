...
def parse_args():
    ...
    p.add_argument("--out", default=None,
                   help="Cache directory. Required unless --report-only.")
...
def main():
    ...
    # Use the SAME loader the trainer uses, not a bare AutoModelForCausalLM.
    # 2026-09-11: the generic call sat at 100% CPU on one core for 17+ minutes
    # with nothing written, on a teacher that load_distillation_teacher brings
    # up in ~13 s in every training log. The trainer's loader does three things
    # this call did not: sets pad_token_id when the Ouro config lacks it, forces
    # attn_implementation="eager" on XPU (Intel's SDPA kernel is documented as
    # segfaulting inside HF models), and moves to the device only after the
    # load. Whichever of those was the hang, the fix is the same — one proven
    # path, not two.
    from mythouro.training_utils import load_distillation_teacher
    teacher = load_distillation_teacher(
        a.teacher_id, len(tok),
        device=a.device, dtype=torch.bfloat16,
        trust_remote_code=a.trust_remote_code,
    )
    if teacher is None:
        raise SystemExit("teacher failed to load — see the error above")
