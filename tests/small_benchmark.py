#!/usr/bin/env python3
"""
Side-by-side training + benchmark of MythOuro vs. a vanilla transformer on a
...
3. Depth-extrapolation sweep at the end: MythOuro is trained at
...
from mythouro import MythOuroConfig, MythOuro
...
class BaselineTransformer(nn.Module):
    ...
    Reuses MythOuro's TransformerBlock (attention + FFN kernels are identical)
...
def train_step(
    ...
    optimizer.zero_grad()
    out = model(x)
    # MythOuro returns (logits, uncertainty); baseline returns logits.
...
def evaluate(
    ...
    `n_loops` is only forwarded to MythOuro; for any other module the kwarg
...
def build_tiny_cfg(vocab_size: int, seq_len: int) -> MythOuroConfig:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--depth-sweep",
        default="1,2,4,8,16",
        help="comma-separated n_loops values for MythOuro depth-extrapolation eval",
    )
...
def main() -> None:
    ...
    print(
        f"[setup] MythOuro params  = {fmt_count(n_m)}  ({n_m:,})\n"
    ...
    print(f"  {'':<24} {'MythOuro':>16}   {'Baseline':>16}")
    ...
    # ------------------------------------------------------------------
    # Depth extrapolation: MythOuro eval loss as a function of n_loops.
    ...
    print(f"  MythOuro (trained at n_loops={cfg.max_loop_iters}):")
