"""
MythOuro vs. vanilla GQA+MoE transformer benchmark.

Compares MythOuro (Prelude + looped Recurrent Block + Coda with ACT halting,
...
non-recurrently. The baseline reuses MythOuro primitives so the comparison
...
Metrics reported:
    ...
    - MythOuro depth-scaling sweep: latency vs. n_loops
...
from mythouro import MythOuroConfig, MythOuro, mythouro_1b
...
class BaselineTransformer(nn.Module):
    ...
    with MythOuro so any speed delta is attributable to the recurrent-depth
    architecture rather than the underlying attention/FFN implementation.
    """

    def __init__(self, cfg: MythOuroConfig, n_layers: int):
        super().__init__()
        self.cfg = cfg
...
def count_params(model: nn.Module, cfg: MythOuroConfig) -> ParamCounts:
...
def get_cfg(size: str) -> MythOuroConfig:
    size = size.lower()
    if size == "small":
        return small_cfg()
    if size == "1b":
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--n-loops",
        default="1,4,8",
        help="comma-separated loop counts to sweep (MythOuro only)",
    )
    p.add_argument(
        "--decode-steps",
        type=int,
...
def main() -> None:
    ...
    # to match the unique-parameter depth of MythOuro (parameter-matched baseline).
    baseline_n_layers = cfg.prelude_layers + 1 + cfg.coda_layers

    torch.manual_seed(0)
    mythos = MythOuro(cfg).to(device=device, dtype=dtype).eval()
    torch.manual_seed(0)
    baseline = (
    ...
    print(
        f"  MythOuro : total={fmt_count(m_params.total):>10}   "
    ...
    for seq_len in seq_lens:
        ...
        for nl in n_loops_sweep:
            ...
            mem = peak_mem_mb(device)
            print(
                f"  {'MythOuro (loops=' + str(nl) + ')':<26} {seq_len:>6} "
                f"{secs*1000:>9.2f}ms {tps:>12,.0f} {mem:>10.1f}"
            )
    ...
    for nl in n_loops_sweep:
        ...
        print(
            f"  {'MythOuro (loops=' + str(nl) + ')':<26} "
            f"{per_step*1000:>10.2f}ms {tps:>14,.1f}"
        )

    # ---- Depth scaling ----
    print_header(
        "MythOuro depth scaling (fixed seq={}, batch={})".format(
