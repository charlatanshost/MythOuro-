...
|---|------|------|--------------------------|
| **A** | **α-anneal 0.45 → 0.40** (`run_anneal.sh`, edit ALPHA) | **~5h — DO THIS NEXT** | **The most productive lever in the project.** 0.5→0.45 took raw code L3+ **65.0 → 82.5%** in 3,500 steps — monotone (76.2 / 80.0 / 82.5), past the old 75.0% record after 1,500 steps, degeneracy flat at 0/80. Also moved α=0.0 prose for the first time in 50,000 steps (top_share 0.139 → 0.092 over four seeds). Chat flat (33.8 → 31.2, inside noise), so it is a raw-frame gain, not a trade. Abort signal: a fragile seed past 0.40 at α=0.0. Control `checkpoints_newmix` untouched; fallback `step_0140000.pt`. |
