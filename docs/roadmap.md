...
[max1100_field_notes.md](max1100_field_notes.md) "GROWTH UNBLOCKED". **No 397M
...
said prose up / code down; and `code_eval` at deployment settings said capability
**up 17.5 points to a project record**. The mean was flat only because prose gains
and code losses cancelled. The rollout probe samples at T=1.0 with NO repetition
penalty — worst-case, not deployment. **Always split by domain, and always confirm
against capability at the settings that will actually be used.**

**⚠ THE HEADLINE METRIC CHANGED 2026-08-18 — report BOTH FRAMES.** Raw-framed
code L3+ was the project's headline number all year. It is now measuring the frame
the model is LEAVING: between ~116,000 and 120,000 the pour crossed into chat-model
behaviour, and raw-frame code fell 75.0 → 55.0 while chat-frame rose 6.2 → 37.5 on
the same weights. Reading either alone inverts the verdict. A local coding
assistant is used in CHAT frame, so that is the number that matches the goal —
but keep both, exactly as the pen-1.0/pen-1.15 protocol keeps both penalties.

**⚠ `best_exit_probe` IS WITHIN-SESSION ONLY.** Its eval text streams from remote
HF datasets. It reproduces to ±0.0012 on a repeat in the same sitting, but step
108,000 read 0.348 where an archived run 471 steps away had 0.2036. Never compare
its absolute CE across sessions — a cross-session comparison produced a false
"the pour degraded the model 1.7-3.5x" alarm on 2026-08-18. `kd_exhaustion` caches
a fixed held-out sample and does not have this problem; prefer it for any series.
...
|---|------|------|--------------------------|
| ~~**A**~~ | ~~α-anneal ladder~~ | ✅ **CLOSED 2026-08-22 — VALIDATED, then exhausted** | At n=320: 0.5 → 0.45 is a REAL **+18pp** (59.1 ±5.4 → 77.8 ±4.6), the only difference measured all week that survives proper power. But 0.40 over-annealed (salad returned, raw fell), and 6,500 further steps at 0.45 regressed on every axis. **Base to carry forward: `step_0143500.pt`.** Prior text below is the n=80 record and its magnitudes are inflated ~5-6pp.
| ~~**B**~~ | ~~Chat-mix retry on the annealed base~~ | ✅ **DONE 2026-08-24 — content moved (raw); chat still broken** | RAW, n=320 vs base: L3+ held (76.9 vs 76.6), `committed` 25% → **42%**, L4 5 → **14/320** (non-overlapping). `reverse_string`, never solved before, now solved. **First thing all year to move content rather than form.** But median `body_stmts` is still 1 and every newly-solved task is a one-liner. CHAT is 0.0% in both legs — see rung C. |
| ~~**C**~~ | ~~`--rollout-len` 64 → 256~~ | ❌ **NULL 2026-08-25** | 1,738 steps at 4x window moved think-block length 381 → 408 words and closure 48 → 46 of 320. The window is not the mechanism. Raw held (78.8 vs 76.9). Cost note: |
| ~~**E**~~ | ~~XPU segfault at 48 experts~~ | ✅ **ROOT CAUSE FOUND 2026-08-28: corrupt persistent SYCL kernel cache** | Promotion works, the sentinel decay in `distill.py` works (123e460), the 48-expert model passes a COMPLETE training step in isolation (peak 21.75 GB of 48), and the 278M control runs the identical path clean. But `distill.py` segfaults deterministically inside the first rollout — `faulthandler` stack: `main.py:851` MoEFFN topk → `generate_rollout` → `rollout_with_retry` → `distill.py:869`. `rollout.py`'s own docstring records the same fault class as a ONE-OFF at 24 experts on 2026-07-12; at 48 it is every time. Not memory, not the model, not the data stream. RESOLVED: `~/.cache/libsycl_cache` held a corrupt JIT kernel binary (oldest entry 2026-07-12, the day of the original one-off abort). Unsetting `SYCL_CACHE_PERSISTENT` — one variable — made the five-times-crashed config run 3 clean profiled steps. Standalone repros passed all along because fresh shells never had the var. Cache quarantined; verification at production config pending; full story in `max1100_field_notes.md`. **Growth is unblocked.** |
| **D** | **EXPAND THE TEACHER CORPUS** (`tools/make_code_corpus.py --source <name>`) | **CPU only — DO THIS NEXT** | **The constraint was token starvation in the teacher stream, and fixing it is the only thing that has ever moved content.** The 20% teacher stream was 10.9M tokens — one epoch every 3,339 steps, re-read dozens of times by step 140,000. Two additions (code 35M, math 58M → 92M, one epoch every 28,229 steps) took multi-statement solutions **3% → 15% → 28%** and L4 **5 → 14 → 26 per 320**, monotone, while `body_stmts` had been median 1 at every α and every checkpoint ever measured. Remaining, all provenance-clean and GPU-free to convert: **OpenMathInstruct-2 ~14M pairs (250k used), OpenCodeInstruct 200k (100k used), PubMedQA 100k (untouched, the mission domain)**. ⚠ Tulu-3 held — chat prose (median 0 stmts) AND its WildChat filter returned 0 drops where ~10% was expected. Superseded rollout note: | A 9-config sweep (`run_bench_rollout.sh`) measured what a complexity argument got wrong: scaling is **~7x at len 256, not the 16x O(n²) predicts**. Per 6,000-step leg against the current 2.1 h (len 64 / batch 32): **len 128 / batch 16 = 2.8 h** (+0.7 h for double the window), **len 256 / batch 8 = 5.6 h** (leg goes ~8.5 h → ~12 h). Both are runnable nights. ⚠ Lower `--rollout-batch` is cheaper partly by generating fewer on-policy sequences (32 → 8 quarters them) — a real trade. ⚠ 256 still does not span a ~495-token think block; 512 / batch 8 extrapolates to ~13 h. An earlier entry called this "not viable" from projection alone and was wrong by >2x in the favourable direction. Rationale for the change: Chat framing is 0.0% and three explanations died: the corpus think-block (fixed, still 320/320 locked), the 96-token eval cap (raised to 512, still 0.0% — the think block just **expands to fill the budget**, 94 tok at 96, 495 tok at 512), and closing-is-enough (48/320 closed, only 5 emitted a fence, all L0). **Every run in project history used `--rollout-len 64`** — the on-policy window has never reached past a ~495-token think block, so it has NEVER trained the exit from reasoning or the answer after it. The 2026-08-17 λ refutation measured the RATE of opening `<think>`, not its length or closure, so it does not cover this. Costs ~4x rollout compute; size it deliberately. |
...
| 3 | **`--loop-loss-weighting`** | ⚠️ **RUNG 3 REOPENED 2026-09-01 — the 08-11 result was a BUG, not a finding.** `forward_loop_states` gated trajectory capture on `not self.training`, so in training it got `last_trajectory=None`, fell through to its `traj = e.unsqueeze(2)` branch (labelled "n_loops == 0"), and supervised **`coda(prelude(x))` with the entire recurrent block bypassed**. Accuracy 0.980 → 0.075 is what training a model to answer from its prelude alone looks like. `uniform`/`progressive` failed SILENTLY; only `exit_pdf` raised, because its shape check caught K=1 against a 4-step halt distribution. Fixed (`main.py`, honour `trajectory_requires_grad`), regression-tested (`tests/test_loop_states.py`), default inference path unchanged. **Ouro's `exit_pdf` has still never been run.** Prior verdict below is retracted. | ~~❌ DEAD 2026-08-11~~ | Per-loop accuracy collapsed at EVERY depth (loop 3: 0.980 -> 0.075) and the trajectory INVERTED — deeper loops now worse than shallower. math L3+ 5.0% -> 0.0%, depth sweep still flat at 4/6/8. The "expected transient" story was falsified by its own stated test: loops 0-2 did not rise. **uniform is the HARSHEST arm, not the control** — it gives loop 0, never an output state, 25% of the gradient. If revisited: LoopFormer shortcut-consistency (2602.11451), or progressive/exit_pdf with depth-reg lowered. |
| 4 | ~~**Resume the pour** to 140,000 (`run_newmix_pour.sh`)~~ | ✅ **COMPLETE 2026-08-20 — reached 140,000** | 14,819 steps left, ~21h at the measured 5.0 s/step = 2 evenings. **The pour is doing something nobody planned:** between ~116,000 and 120,000 it crossed a phase transition from a continuation model into a CHAT model — chat-framed code L3+ 6.2% → 10.0% → **37.5%** with ZERO chat data, while raw-framed code fell 75.0 → 55.0 and soft-KL to the teacher kept falling straight through. Converging on Ouro-2.6B-**Thinking** means exactly this. Three SFT attempts and two chat-mix legs failed to produce instruction-following; the pour produced it for free. **This is the only thing that should be running.** The base is still the binding constraint on everything else; every other rung is a better *objective* on the same weak base, and 2026-08-17 added a second reason — the `<think>` failure (rung 9) has no known fix, so capability is the axis that is actually movable right now. Eval at ≥2 milestones, not just the endpoint. |
| 5 | **Grow depth 4→8** (`grow_depth.py`) | ⏸ **SHELVED — and 2026-09-03 CONFIRMS the gate empirically.** The depth sweep on a model that genuinely uses depth (halt 3.21/4, learned, stable) is FLAT at 4/6/8: L3+ 75.6 / 76.2 / 75.6. ρ(A) mean 0.289 means the linear term is 0.7% of initial by loop 4, and NOTHING in `RecurrentBlock` scales with loop count — loop 1 and loop 8 are treated identically. Adding loops without DeepLoop's loop-dependent residual scaling adds them into a regime measured as empty. Order is: residual scaling FIRST, then grow_depth, then re-sweep. | The 2026-08-14 per-loop CE curve (0.212 at loop 3 → 0.640 at loop 7) measures UNTRAINED extrapolation — loops 4-7 reuse slot 3's adaptation and are off-distribution by construction. It says nothing about TRAINED deeper loops, which is what this tool creates. Gated on a base worth spending nights on, plus DeepLoop's residual-scaling recipe ([2607.13491](https://arxiv.org/abs/2607.13491)). |
