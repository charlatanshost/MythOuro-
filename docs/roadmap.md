...
| ~~**C**~~ | ~~`--rollout-len 64` → 256~~ | ❌ **NOT VIABLE 2026-08-24 — measured, not estimated** | `tools/bench_rollout.py` on the real card: rollout-len 64 costs **11.74 s** per rollout (174 tok/s), and a 6,000-step leg regenerates 750 times = **2.4 h, 29% of an 8.5 h leg**. Uncached decode is O(n²), so 128 ≈ 9.8 h and 256 ≈ 39 h of rollout generation alone. Not a hardware ceiling — rollouts are hard-pinned uncached because cached decode runs FULL depth (both early-exit paths require `kv_cache is None`) against the measured ACT depth of 2.00/4, a gap worth 0.95 nats. **The window cannot span a ~495-token think block; the reasoning has to get shorter instead.** Original rationale: Chat framing is 0.0% and three explanations died: the corpus think-block (fixed, still 320/320 locked), the 96-token eval cap (raised to 512, still 0.0% — the think block just **expands to fill the budget**, 94 tok at 96, 495 tok at 512), and closing-is-enough (48/320 closed, only 5 emitted a fence, all L0). **Every run in project history used `--rollout-len 64`** — the on-policy window has never reached past a ~495-token think block, so it has NEVER trained the exit from reasoning or the answer after it. The 2026-08-17 λ refutation measured the RATE of opening `<think>`, not its length or closure, so it does not cover this. Costs ~4x rollout compute; size it deliberately. |
