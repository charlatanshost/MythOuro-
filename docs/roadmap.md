...
| **B** | **Chat-mix retry on the annealed base** (`run_chatmix.sh`, SRC → `checkpoints_anneal045/step_0143500.pt`) | **~5h — DO THIS NEXT** | The anneal fixed FORM; nothing has ever moved CONTENT (strict L4 0-2/80 since step 64,000, median `body_stmts` **1** at every α ever measured). α/λ/loop-weighting/dose all reshape the sampling distribution — none add task competence, because the model trained on CONTINUATION data and nothing rewarded finishing a task. The 26,130-row `data_teacher_chat/` corpus is the only asset that demonstrates COMPLETION. The two chat-mix legs failed on a base that was degenerate; this base is 0/80 degenerate, salad-free and restarts a quarter as often. Gate: `committed` and `body_stmts`, NOT L3+. |
