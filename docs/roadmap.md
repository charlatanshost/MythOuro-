...
| **E** | ⚠️ **GROWTH IS BLOCKED — XPU segfault in rollout at 48 experts** | **unresolved 2026-08-28** | Promotion works, the sentinel decay in `distill.py` works (123e460), the 48-expert model passes a COMPLETE training step in isolation (peak 21.75 GB of 48), and the 278M control runs the identical path clean. But `distill.py` segfaults deterministically inside the first rollout — `faulthandler` stack: `main.py:851` MoEFFN topk → `generate_rollout` → `rollout_with_retry` → `distill.py:869`. `rollout.py`'s own docstring records the same fault class as a ONE-OFF at 24 experts on 2026-07-12; at 48 it is every time. Not memory, not the model, not the data stream. Full elimination table in `max1100_field_notes.md`. **Do not plan around growth until resolved.** Next: `--onpolicy-lambda 0` (removes rollout entirely) and auditing `rollout_with_retry`, which the repro never exercised. |
