...
| 2026-07-23 · **Triaged OUT — recorded so they aren't re-litigated** | n/a | **(a) "Scaling Quantum-Inspired LLM Fine-Tuning Research with H200 GPUs"** (mondrian.ai / Korea Univ AI & Mobility Lab) — a lab/vendor story, not a paper, and **unreadable from here (expired TLS certificate)**, so this is a title-level judgement only: "quantum-inspired fine-tuning" normally means tensor-network (MPS / tensor-train) PEFT adapters. We already run LoRA-per-loop; an H200-framed marketing piece is not evidence to switch. Un-parks if a real paper with a clean license appears. **(b) "Agent-Specific Prompt Engineering for LLM-Guided RL Exploration"** (B. Gu et al., IEEE 11480828) — prompting an LLM to propose actions/subgoals that steer an **RL agent's** exploration. Fails triage #1: we do not do RL. Our on-policy/GKD is teacher-driven *distillation*, not reward-based RL, and the ideas.md position is already that RL slots in only past coherence and suits *verifiable* rewards — which open medical diagnosis is not. This is also LLM-guides-RL-agent, a different problem from RL-trains-LLM. **(c) "Yonsei University Boosts Boltzmann Machine AI Using Quantum Computing"** (dongascience 77880) — popular-press coverage of **arXiv 2604.00919 above**, same Yonsei group; no additional technical content. |
