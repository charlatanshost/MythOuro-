...
| 2026-07-23 · **"Scaling Quantum-Inspired LLM Fine-Tuning Research with H200 GPUs"** (mondrian.ai / Korea Univ AI & Mobility Lab) | **UNREAD — pending access.** PEFT / adapter-efficiency questions | ⚠ **No verdict recorded: the page is unreadable from here (expired TLS certificate) and I have only the title.** On the title alone, "quantum-inspired fine-tuning" usually denotes tensor-network (MPS / tensor-train) low-rank adapters — which would be the same slot as our LoRA-per-loop, i.e. a potential *alternative* parametrisation of the per-loop adapters rather than a new axis. That is a guess, not a finding. **Un-parks when:** the page is readable, or a primary paper with a checkable license surfaces. Worth a second look precisely because per-loop adapter parametrisation is load-bearing for us and we've only ever tried LoRA. |
