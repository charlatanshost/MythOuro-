...
| 2026-07-17 · **Teacher-generated synthetic data** — use Ouro to generate clean training tokens | **Token supply** (#1 bottleneck) | Medium (gen + curate) | Yes | Backlog — strong; **design written: docs/teacher_corpus_plan.md** (gated on 18k probe) | Attacks token *supply*, not just efficiency; fully OpenAI-free. Pairs with phi-style quality. |
| 2026-07-17 · **Sequence-level KD** (Kim & Rush 2016) — teacher generates sequences offline, student trains on them | Both (cheap exposure-bias help) | Low–Med (offline gen, no per-step sampling) | Yes | Backlog — Tier-2 stepping-stone; **merged into docs/teacher_corpus_plan.md** | Cheaper precursor to full on-policy; no RL loop. |
