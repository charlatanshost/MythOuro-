...
## Shipped — implemented ideas and what they measurably did

The scoreboard the backlog feeds. Full evidence lives in generation_probe_tracker.md /
training_runs.md; this is the one-line-per-idea outcome record.

| Idea (logged) | Shipped | Outcome |
|---|---|---|
| 2026-06-18 · **On-policy GKD + MiniLLM (Tier-1 rev-KL, then Tier-2 rollouts + teacher-mix)** | Tier-1 2026-06-16 · Tier-2 2026-06-27 | **THE collapse cure — the project's biggest win.** rev-KL gave the stable 6675 base; on-policy rollouts un-collapsed α=0.0 (first movement ever), then two regime shifts (6906 varied-salad, 8668 grammatical English). Still the training recipe's core. |
| 2026-06-18 · **Sandwich norm + depth-aware init** (Huginn recipe) | in the main recipe since the 6675 rebuild | Part of the stable-base recipe (`--use-sandwich-norm --use-depth-aware-init`); no instability since. Cannot be isolated from the rev-KL contribution — shipped as a package. |
| 2026-06-18 · **recurrent_state_noise** | tested 06-15/16 | **Marginal — retired.** Moved 1→2-word repeats only; the real cure was on-policy. Kept as a knob (σ=0 default). Honest negative. |
| (unlogged; built from need) · **Phase-5 batched+cached rollouts** | 2026-07-14 | **Mixed, high-value.** Wide batching = the real win (11.7× effective rollout throughput, kept). Cached student decode = NOT distribution-preserving (~1 nat KL; ACT early-exit granularity) → corrupted steps 9780→12000, caught by probe 07-16, pinned uncached. Paid for the KL-gate lesson. |
| (unlogged; built from need) · **--min-lr cosine floor** | 2026-07-18 | Both long legs' final ~4k steps had been silently starved at lr·0.1; the floor made the 18k→30k leg the first properly-powered frontier test. Small code, large attribution value. |
| (unlogged; built from need) · **--no-kv-cache probe instrument** | 2026-07-16 | Restored comparability with all pre-07 probe entries; revealed ~half the apparent 12000 collapse was instrument artifact. |

## In flight — implemented, results pending

| Idea (logged) | Status | The pending question |
|---|---|---|
| 2026-07-17 · **Teacher-generated synthetic data + sequence-level KD** (one build — docs/teacher_corpus_plan.md) | Pipeline shipped 07-18/19 (generator, XPU-safe decode, calibrated filters, `--teacher-data-ratio`); harvest running ~4.8M tok/day | **The R=0.2 A/B** (launch ~07-20/22): does teacher-clean text move the plateaued α=0.0 probe where more web text couldn't? Positive → data quality was the wall (lean in: rephrase mode next). Flat → wall is likely model size → v6 SFT / scale-up. |

## Backlog

Every table row below leads with its **logged date** (backfilled from git
history 2026-07-19; the 06-18 cluster is the original consolidation). Date new
entries on the way in.

| Idea | Targets | Local test cost | Reversible | Status | Notes |
|------|---------|-----------------|-----------|--------|-------|
| 2026-06-18 · **Sandwich norm + depth-aware init** (Huginn recipe; corroborated by Ouro) | **Scale-up stability** (NOT the current bug) | staged code, config-gated | Yes (default off) | **STAGED, RE-SCOPED 2026-06-17** — `--use-sandwich-norm --use-depth-aware-init`. NOT for the current exposure-bias spiral (reps healthy → demoted there). **Re-elevated for the SCALE-UP fresh distill.** | Ouro (teacher) + Huginn BOTH use sandwich norm for recurrent stability at scale; we're the pre-norm outlier. Adopt when scaling, not now. training_runs.md 06-17. |
| 2026-06-18 · **Multi-Token Prediction (MTP)** (DeepSeek-V3: aux multi-token heads in training, discarded at inference) | Token efficiency (extra training signal) | Medium (add heads + a distill run) | Yes (aux objective, removable) | Backlog | Fits our DeepSeek-V3 routing stack. Enhancement, *not* a collapse fix. From the NTP-alternatives survey (arXiv 2509.24435). |
| 2026-06-18 · **Data quality (phi-style "textbooks")** (curate higher-quality distill corpus) | Coherence per token | Medium (corpus work) | Yes | Backlog | Small models get coherent on curated data at fewer tokens. Stay OpenAI-free (the clean-data constraint). |
| 2026-06-18 · **Unlikelihood training** (Welleck et al. 2019) — loss term penalising high prob on repeated tokens | Collapse | Low (aux loss term) | Yes | Backlog — strong cheap lever | Surgical anti-repetition; could layer onto the current run. |
| 2026-07-19 · **Narrative/stories data slice (~10%)** (TinyStories lesson: narrative teaches discourse coherence per-token better than web exposition) | Coherence per token (the stalled probe metric) | Low (dataset swap + one A/B leg) | Yes (mix knob) | Backlog — **queued BEHIND the teacher-corpus A/B** (one data variable at a time) | Rationale 2026-07-19: at 278M breadth costs depth, so no general variety expansion — but narrative is the one slice targeting the exact stalled metric. Candidate source must pass the clean-data constraint (no TinyStories itself — OpenAI-generated; vet alternatives e.g. BookCorpus-open/Gutenberg via dataset_selection.md). Test: same protocol as teacher-corpus A/B — one leg, uncached n=5 probe vs baseline. |
| 2026-07-19 · **Harvest rephrase mode** (BeyondWeb/WRAP lesson: teacher REPHRASES full real docs instead of continuing seeds) | Corpus quality per token (the plan-B axis) | Low (prompt-side change in gen_teacher_corpus; same pipeline) | Yes (mode flag) | Backlog — **first upgrade if the R=0.2 A/B is positive** | arXiv 2508.10975: rephrasing beats free generation for pretraining and resists mode collapse; keeps info anchored to real docs, cleans style. Also add Cosmopedia-style frame diversity (audience/format conditioning) while in there. |
| 2026-07-19 · **Loop-skip self-speculative teacher decode** (draft at 1 UT loop, verify at 4 — looped models get the draft model for FREE) | Harvest throughput (launch-bound decode is our bottleneck) | Medium (draft/verify + top-p rejection-sampling math in the existing custom decode loop) | Yes (new path, old path kept) | Backlog — between-experiments build | Output distribution provably unchanged (speculative decoding guarantee). Verification of K tokens = ONE parallel forward = converts the Max's worst workload shape (sequential 1-token steps) into its best (wide parallel). Plausible 2-3x on the measured 56 tok/s. ConfLayers (2604.14612) is the layer-skip analog. |
| 2026-06-18 · **Tokenizer graduation** (transplant vocab → re-match to a bigger teacher family) | **Capability ceiling** (unlocks bigger teachers) | High (transplant + heal) | Partially (new run, but gated) | **Future milestone** — gated on (a) Ouro saturated + (b) base structurally settled | The *bridge between two matched-KD regimes*: migrate the matched-vocab anchor from SmolLM2/Ouro (small teachers) to e.g. Qwen (big teachers). NOT cross-tokenizer KD (lossy, keeps vocab) — this re-matches so clean logit KD resumes. See deep-dive below + roadmap "Tokenizer graduation". |

## Parked (failed triage — recorded so we don't re-litigate)

| Idea | Why parked | What would un-park it |
|------|-----------|----------------------|
| 2026-07-23 · Boltzmann-GPT-style energy world model (arXiv 2601.17094 — DBM over structured features → soft prompts → frozen LM; "the mouth is not the brain") | **Needs a fluent mouth first** — their frozen GPT-2 had 40B tokens of language; ours is mid-babble, so conditioning machinery solves the wrong bottleneck today | **A coherent, instruction-following base (post-v6 SFT) + the MEDICAL phase.** Then it slots beside MemPalace as the *parametric* complement to retrieval: an energy model over clinical features (drug-interaction / symptom-disease co-occurrence) = a **plausibility veto** the generator cannot hallucinate past — the honest-specialist thesis as mechanism. Tiny (31K-param DBM + MLP adapter), fully reversible, rig-friendly; soft prompts enter naturally via our injection stream. **Kinship note (2026-07-23):** our recurrent loop IS iterative latent refinement to a fixed point (DEQ lens, ρ(A)<1; ACT = stop at the minimum) — i.e. the loop already descends an implicit energy landscape. Two integration depths when unparked: shallow (paper's soft-prompt conditioning) or deep (couple the world-model energy into the loop's refinement so implausibility is intrinsic, not bolted on). **Design refinement (2026-07-23, after vetting a deep-integration sketch):** Gibbs-in-the-loop is ruled out on our own evidence — attractor dynamics + energy-style halting are things the loop ALREADY does deterministically (ρ(A)<1 + ACT), and in-loop stochastic smoothing is the retired recurrent_state_noise knob (tested, marginal). The keeper is **energy as a CRITIC**: evaluate the world-model energy on the loop's *settled* state (once per token/answer, cheap) as a veto/flag — the guardrail without the intractable sampling. **✅ PAPER READ IN FULL (2026-07-23 evening) — three corrections to the row above.** (Niimi, sole author; DBM + adapter + frozen GPT-2-medium; domain = Amazon smartphone reviews.) **(a) The energy is NEVER used as a critic in the paper — our idea is genuinely novel, not a refinement.** Their conditioning path is entirely mean-field belief state → MLP adapter → K soft prompts; the energy function appears only in Experiment II as *post-hoc analysis* showing higher energy for implausible brand–price combinations, and never constrains generation. That is *better* for us: the paper supplies exactly the evidence our veto needs (this energy discriminates plausible from implausible configurations) while leaving the mechanism unbuilt. So the integration ladder has THREE rungs, not two: paper (energy unused at generation) → **our critic** (energy vetoes the settled state — novel) → full Gibbs-in-loop (already ruled out). **(b) The kinship is tighter than "descends an implicit energy".** Their inference rule is `μ^(l) ← σ(W^(l)ᵀμ^(l−1) + W^(l+1)μ^(l+1) + c^(l))` iterated to a fixed point — *structurally our recurrent loop*. Both are weight-shared fixed-point iteration over a latent state; theirs merely has the energy written down. Also confirms our Gibbs ruling-out independently: the paper runs **no MCMC at generation** either (mean-field to convergence, then a single projection — deterministic feed-forward). **(c) The stated limitation lands in our favour.** The authors flag that the model *"only captured the co-occurrence structure between attributes"* and call temporal dynamics future work — but our target use (drug-interaction, symptom-disease co-occurrence) IS co-occurrence, so its demonstrated capability fits our case better than its own framing. **Against us:** single-domain validation (one product category, one language); authors themselves suggest **VAE or Modern Hopfield Networks** as alternatives ("VAE may enable more stable training") since DBM training is three fiddly stages (layer-wise CD pretrain → joint PCD → mean-field inference); and **K (soft-prompt count) and hidden-layer sizes are never reported** — partial reproduction blind spot. **Gate re-validated:** their frozen LM is GPT-2-*medium* (~355M — our parameter scale, vastly more training tokens), and the paper's thesis is precisely "small LM + world model = controllable generation" = our thesis. Confirms "needs a fluent mouth first" is the right gate. **❌ DEEP-INTEGRATION SKETCH VETTED AND REJECTED (2026-07-23 evening).** An LLM-generated `EnergyConstrainedRecurrentLayer` (GRUCell + inner gradient-descent on `E(h) = −½hᵀWh − bᵀh + ½‖h−x‖²`) was reviewed and RUN. Three findings, in ascending importance. **(i) The reference implementation is broken:** `h_refined = h.clone().detach().requires_grad_(True)` severs the graph, so the GRUCell receives *no* gradient — verified empirically, all four `rnn_cell` params report `grad=None` and move **exactly 0.0** after 20 optimizer steps; only the energy params train. Fixable, but it means any reported training curve is the energy term compensating for a frozen random projection. **(ii) The stability claim is wrong:** "symmetric W with non-negative diagonals ⟹ monotone energy decrease" is a result about *discrete asynchronous Hopfield updates* and does not transfer to gradient descent. The real conditions are `λ_max(W) < 1` (else E is unbounded below) and `lr < 2/λ_max(I−W)`. Nothing constrains W's spectrum, and it is stable at init only because `randn*0.02` is small — verified: at λ_max=0.099 it converges, at λ_max=4.97 ‖h‖ hits **1.6e8 in 60 steps**. Note this is the SAME contractivity condition we already track as ρ(A)<1. **(iii) The decisive objection — it isn't a world model.** The energy is over the *hidden state* with a learned W, trained on the same data as everything else. The Boltzmann-GPT value comes from an energy over structured **external** features; an energy over our own latents encodes nothing the loop doesn't already know, so this adds a second relaxation to a loop that already relaxes to a fixed point — precisely the redundancy this row identified, while delivering none of the plausibility-veto payoff. **Worth keeping from it:** symmetry enforcement `W = ½(W+Wᵀ)`, and the `½‖h−x‖²` anchor binding the latent to token context. (The refinement is *not* cosmetic — measured ~11% movement of h at the sketch's defaults.) **Conclusion unchanged and now twice-tested: energy-as-critic over structured clinical features, not energy-in-the-loop over latents.** |
| 2026-06-18 · Grow the model (more params via 8-bit Adam) | Fails #1 — token-starved (~170× undertrained); bigger model = hungrier, worse | Token-curve shows we've reached compute-optimal at current size **⚠ UN-PARKED IN AUGUST WITHOUT THIS CONDITION BEING MET, AND IT FAILED (2026-09-01).** Growth ran 2026-08-27..09-01 on the regression-wall evidence, not on a token curve. Result: two function-preserving promotions to 48 experts, both ending with the new experts as ~90% twins of their parents on an asymptotic curve (fit 0.893); initialisation ruled out (router perturbed to cos 0.704 differentiated *slower*), load balancing ruled out (removing it moved the curve −0.003). Activated params never moved off **180,726,115** at either 24 or 48 experts, so expert-count growth could not raise the quantity the capacity argument identifies. Live, the grown model was strictly WORSE — L0 15% → 31%, L3+ 68.8% → 48.4% — because 24 undertrained experts absorb half of every token's routing; masking them (`tools/mask_new_experts.py`) restores it entirely. **This row was right the first time.** The un-park condition still stands and is still unmet: the 2026-09-01 prose readout shows the model STILL gaining from tokens (masked 0.104 vs base 0.137 top_share), i.e. not compute-optimal. **✅ CONDITION MET 2026-09-15.** The token curve ran (main-thread #2, 3 legs × 49.2M tokens, α=0): L3+ 79.4 / 78.8 / 75.0 and distinct1 0.576 / 0.562 / 0.556 across points 1–3 — flat across three points at ~16 tokens per activated parameter. **Un-parked, on the right axis this time:** `mythouro_distill_mid` moves dim (1280→1792), prelude/coda (2→4) and top-k (4→6), the three things that change ACTIVATED params — 460M, 2.55x — and leaves the 24 experts alone. Step time at that size is measured by `run_scale_profile.sh` before a night is committed. |
| 2026-06-18 · Pivot to diffusion / LCM / non-transformer (from the NTP survey) | Fails #2/#3 — frontier big-lab work; throws away working stack + teacher | A funded team + compute; not a solo move |
| 2026-06-18 · Pause/think tokens (Goyal et al., arXiv 2310.02226) — extra *width* scratchpad before output | **Already covered** — we do adaptive "think before you speak" via recurrent *depth* + ACT (stronger: adaptive vs fixed-K). Combining width+depth is tangential complexity | A task that needs working-*space* (more hidden vectors), not just iteration-*time* |
| 2026-06-18 · T-FREE tokenizer-free LLM (Deiseroth et al., arXiv 2406.19223) — hash/trigram sparse embeddings | **Breaks distillation** — no shared vocab → can't KL against Ouro's ~49k-token output space → forces train-from-scratch (= abandons the teacher, our token-efficiency lever). Tempting embedding savings (~⅓ of a 278M model) + 3× micro-batch don't outweigh losing the teacher | A *from-scratch* foundation-model project at real scale (not MythOuro) |
| 2026-06-18 · Coconut / Chain-of-Continuous-Thought (Hao et al., arXiv 2412.06769) — latent reasoning ACROSS positions (feed hidden state back as next input) | **Right family, wrong phase.** Orthogonal axis to ours (across-position vs our across-depth latent reasoning) — *combining them is an exciting far-future direction* (cf. Huginn's "warm-start recurrent state from prev token"). But Coconut's Stage 0 needs a base that already does coherent *language* CoT; we're ~1000× tokens away from coherent text. Doesn't touch collapse/tokens; needs CoT data + n+1 sequential passes. | A coherent, CoT-capable base exists → then test depth×position latent reasoning |
| 2026-06-18 · **Parallel looped paths** (owner's original idea) — N independent recurrent paths on the same input, per-token confidence arbitration via the uncertainty head | **Reframed (06-16): a quality / parallel-compute feature, NOT a collapse fix or a speedup.** Real systems insight: recurrent depth is *sequential* (can't parallelize across depth) → independent paths fill otherwise-idle parallel hardware at ~constant latency, arbitrated by the existing best-of-trajectory/uncertainty machinery. Quality-for-compute trade. **Crux: paths must be *deliberately diverse* (injection schedule / routing temp / loop budget / seed)** or they collapse to correlated copies and buy nothing. Plausibly-original combination for recurrent-depth. NOT for training throughput (2× compute = slower training). | A **coherent base** exists → test-time-compute / calibration lever. **Design note written: docs/parallel_loops.md** (incl. the saturate-stranded-decode-compute rationale + honest prior-art). |
...
|----------|---------------|-------|
| 2026-06-18 · [Awesome-Collection-Token-Reduction](https://github.com/ZLKong/Awesome-Collection-Token-Reduction) | **Deployment / throughput phase** (after we have a coherent model) | "Token *reduction*" = fewer tokens for *compute* efficiency (pruning/merging, KV-cache & prompt compression), mostly inference & vision/VLM. NOT "token *efficiency*" (capability-per-training-token = our bottleneck, see GKD). The KV-compression slice is largely already covered by our **MLA**. |
| 2026-06-18 · **Huginn (Geiping 2025, arXiv 2502.05171; code seal-rg/recurrent-pretraining; model tomg-group-umd/huginn-0125)** | **MINED → see training_runs.md 06-16** | Closest cousin to MythOuro. Documents our exact collapse + the cure (LR↓, sandwich norm, depth-aware init). Token reality-check: 3.5B/800B tokens. Promoted to a backlog item (Huginn stability recipe). |
| 2026-07-23 · **⚡ Efficient Parallel Samplers for Recurrent-Depth Models / connection to diffusion LMs** (Geiping, Yang, Su — arXiv 2510.14961) | **HIGH — decode throughput, our exact architecture. Read next.** | Same author as Huginn, aimed squarely at recurrent-depth models: a **"diffusion forcing" sampler** decodes a new token at every forward pass *while* the recurrence keeps refining earlier tokens' latents — reframing recurrent-depth models as "continuous, though causal, diffusion language models". Claim: **up to 5× on a 3.5B model, no fine-tuning.** Two candidate applications, and the first is the surprising one: **(a) the Ouro TEACHER decode = our harvest bottleneck** (Ouro is a looped/UT model with 4 loops, so it may be in scope) — 5× would dwarf every lever in harvest_speedup_plan.md combined (compile +10%, continuous batching ~1.28×); **(b) student inference** at deployment. **⚠ Almost certainly RISKY in our taxonomy** — parallel token emission is not autoregressively distribution-preserving the way prealloc-cache is, so it needs the blocking KL/argmax equivalence gate before *any* corpus is harvested with it (cf. the 07-16 cached-decode incident). Also ties to `parallel_loops.md` and the speculative-decode lever (both are "spend idle parallel hardware on a sequential bottleneck"). **First task: read it and decide whether the sampler applies to Ouro's UT cache or only to Huginn-style prelude/recurrent/coda stacks.** |
| 2026-07-23 · **Multi-Mode Quantum Annealing for Generative Representation Learning with Boltzmann Priors** (Kim & Park, Yonsei — arXiv 2604.00919v2) | **Energy-thread background only — not actionable** | VAE with a *general* (non-restricted) Boltzmann-machine prior, sampled by **quantum annealing on D-Wave Advantage2** (up to 2000 qubits), with three annealing modes (training / unconditional / conditional generation). Beats Gaussian-prior VAEs on MNIST/Fashion-MNIST/CelebA; OOD detection via **energy score** (AUC 0.757 vs 0.744 KL) — i.e. independent evidence that *a learned energy is a good plausibility score*, which is the premise of our energy-as-critic idea. **Blocked for us on hardware** (needs a D-Wave annealer; the whole point is that PCD/Gibbs on classical hardware is what they're escaping). Useful as: (a) corroboration that non-restricted Boltzmann priors are trainable at scale, (b) the "train once, condition many ways" framing — one learned energy landscape serving multiple downstream tasks without retraining, which is the shape we'd want for a clinical energy model. Press coverage: dongascience.com/en/news/77880 ("Yonsei University Boosts Boltzmann Machine AI Using Quantum Computing") — same group, no extra technical content. |
| 2026-07-23 · **CANNs: A Toolkit for Research on Continuous Attractor Neural Networks** (He, Tuerhong, She, Chu, Wu, Zuo, Wu — Peking Univ, arXiv 2606.27783) | **Analysis technique, not a method** — background for the attractor/energy thread | Computational-*neuroscience* toolkit (BrainPy/JAX + Rust) for grid-cell / head-direction circuits — **not an LM paper**, and the Wu–Amari–Wong dynamics don't transfer. Two things are worth knowing: (a) their energy visualisation `E(r) = −½rᵀWr + Iᵀr` is the same quadratic form every sketch in this thread keeps reinventing; (b) **ASA (Attractor Structure Analyzer)** applies *persistent homology / topological data analysis* to spike trains to detect ring and toroidal attractor signatures in real recordings. That last one is a genuine candidate instrument for **our own loop**: point TDA at the latent trajectories across UT loops to characterise the attractor geometry, alongside `tools/collapse_metrics.py`. Would answer "what SHAPE is the fixed-point basin our recurrence settles into" — a question we've only ever answered with scalar proxies (ρ(A), rank, top_share). Low priority, no bottleneck; logged so the instrument is findable when the base is coherent enough for the question to matter. |
| 2026-07-23 · **Protein Language Model Embeddings Improve Generalization of Implicit Transfer Operators** (arXiv 2602.11216; ICML 2026 poster) | **Wrong domain — logged for ONE conceptual inversion** | Molecular dynamics, not language. ITO = conditional denoising-diffusion surrogates for stochastic MD at time steps ~6 orders of magnitude larger than real MD; TITO is the chemically-transferable version; this paper adds protein-LM embeddings as auxiliary conditioning to improve data efficiency and generalisation. **Why it's here:** it is the exact *inverse* of Boltzmann-GPT — a pretrained **LM conditioning a dynamics/world model**, where Niimi has a **world model conditioning an LM**. Same coupling, opposite direction, and both report the coupling improves generalisation. Also the natural literature if we ever want a *dynamical* world model instead of a co-occurrence one — which is precisely the limitation Niimi flags as future work (MD is Boltzmann-distribution sampling, so it stays in the energy family). Nothing actionable at 278M; a pointer for the medical phase if "co-occurrence structure" ever proves too static. |
| 2026-07-23 · **The Recurrent Temporal RBM captures neural assembly dynamics in whole-brain activity** (Quiroz Monnens, Peters, Hesselink, Smeets, Englitz — Radboud Univ, *eLife* 2024-11-05, reviewed preprint 98489v2 / PMC11537485) | **Energy-thread background — but it answers the limitation row 77 flagged** (filed late 2026-08-10; sent 07-23, missed at the time) | Computational *neuroscience*, not an LM paper: whole-brain zebrafish light-sheet recordings (8 animals, ~40k neurons each). **Why it belongs in this thread and not the bin:** row 77 records that Niimi's Boltzmann-GPT *"only captured the co-occurrence structure between attributes"* and calls temporal dynamics future work; row 102 (ITO) was logged as the pointer for *"if co-occurrence structure ever proves too static."* **RTRBM is the direct answer inside the same family** — an RBM with recurrent weights `U` connecting hidden units across time steps, i.e. the temporal extension of exactly the prior Niimi uses, and **classical** (unlike 2604.00919, which escapes classical Gibbs by requiring a D-Wave annealer). Two transferable pieces: **(a) the staged training recipe** — fit a compositional RBM first to get sparse, localised assemblies, then *transfer-learn* the temporal weights on top, keeping the assemblies interpretable. For a clinical energy model that maps to: learn the co-occurrence structure (symptom–disease, drug–interaction) first, then add trajectory dynamics over it, without retraining the structure. **(b) The honest negative:** their headline gain is on *time-shifted* statistics (lower nMSE predicting ahead in time) — recurrence buys you temporal prediction and buys nothing if the target is static co-occurrence, which is what our drug-interaction / symptom-disease case actually is today. **So this un-parks strictly AFTER energy-as-critic exists and only if patient-trajectory (over-time) plausibility becomes the question** — it is a second-order refinement of an idea that is itself gated on a fluent base. **Against transfer:** validated on spontaneous neural activity in one organism, no language or clinical data anywhere; RBM training is the same three fiddly stages row 77 already flags (and the authors add a transfer stage on top); assemblies are discovered, not specified, so nothing guarantees a clinically meaningful factorisation. |
| 2026-07-23 · **Agent-Specific Prompt Engineering for LLM-Guided RL Exploration** (B. Gu et al., IEEE 11480828) | **The post-coherence RL stage** — same slot as VibeThinker/MGPO and DistiLLM-2 scheduling | Prompting a strong LLM to propose actions/subgoals that steer an **RL agent's** exploration. *Not* our problem today (we do no reward-based RL; on-policy/GKD is teacher-driven distillation), and it's LLM-guides-RL-agent rather than RL-trains-LLM. **But the transferable fragment is real and I nearly missed it:** "how do you condition the guide so its guidance actually improves the learner's exploration" is structurally the question behind our **`--teacher-mix-alpha`** — a stronger model steering a weaker learner's sampling distribution. We currently set α by hand (0.5–0.7) with no principled schedule; DistiLLM-2's adaptive scheduling is already shelved for the same gap. **Un-parks when:** we revisit α scheduling on on-policy rollouts, or reach the verifiable-reward sub-tasks (dosing math, interaction lookups, guideline adherence) where RL is legitimately in scope. |
| 2026-07-23 · **Quantum-inspired PEFT cluster — QAA / QuanTA / QTHA** (QAA: Korea Univ AI & Mobility Lab, **CIKM 2025**, doi 10.1145/3746252.3760795; survey arXiv 2509.16244 · **QuanTA** arXiv 2406.00132 · QTHA arXiv 2503.12790) | **Rung-2 scale-up** (grow the student), or if per-loop adapter *capacity* ever becomes the limiter | Owner supplied the source text 2026-07-23; primary papers then located. **The slot is exactly ours** — these all replace the linear adapter that LoRA-per-loop occupies, claiming more expressivity per trainable parameter. **Why it could matter MORE to us than to a plain transformer:** we pay the adapter cost **once per UT loop**, so any per-adapter parameter saving *multiplies by loop count*. That is a genuine structural fit, not a generic PEFT pitch. **⚠ But QAA specifically is blocked on our stack.** It is not merely "quantum-inspired": it does amplitude embedding into a Hilbert space, transforms via a **parameterized quantum circuit**, and optimises through the **parameter-shift rule** — i.e. real/simulated quantum circuitry in the training loop. On a Max 1100 via `torch.xpu`, where the field notes already record segfaults on stock ops (`topk`/`multinomial`), bolting a PQC simulator (PennyLane/Qiskit) into the step is a large integration risk for a benefit that is *not* our bottleneck — tokens and corpus quality are. Parameter-shift gradients also cost ~2 circuit evaluations **per parameter**, a real training-time tax. **→ The actionable member of the family is QuanTA** (arXiv 2406.00132, "Quantum-Informed Tensor Adaptation"): *quantum-informed but classical* — tensor operations on reshaped axes giving high-rank adaptation with no quantum hardware or simulator, so it drops into our existing stack. QTHA (QNN + tensor network) reports **76% fewer trainable params than LoRA at preserved performance**, the only concrete number in the cluster, but is quantum again. **Un-parks when:** we grow the student (Rung 2), where parameter efficiency becomes load-bearing — or sooner if per-loop specialisation looks adapter-capacity-limited rather than token-limited. **Read QuanTA first; treat QAA/QTHA as the theory behind it.** Check licenses at adoption. |
| 2026-07-23 · **oFFN: Outlier and Neuron-aware Structured FFN for Fast yet Accurate LLM Inference** (ASPLOS '26 — doi 10.1145/3779212.3790194) | Deployment / inference-throughput phase | Structured FFN sparsity exploiting outlier + neuron awareness for faster LLM inference. Same shelf as linear-CE and the token-reduction collection: an inference-time efficiency lever for *after* we have a coherent model, orthogonal to the recurrent core. Paywalled (ACM DL 403 from here) — abstract only so far. |
| 2026-06-18 · **linear_cross_entropy_loss** (Geiping, github.com/JonasGeiping/linear_cross_entropy_loss) | Throughput/VRAM phase | Fused head+CE, avoids materialising logits → bigger batch. **Caveat:** distill KL needs full logits → useful for hard-CE/SFT terms, not the distill soft-loss. |
| 2026-06-18 · **flash-attention** (Dao-AILab) | **Blocked now** | FA2 unsupported on cuda_cc (12,0) = Blackwell 5070; we're on SDPA fallback. Watch for FA3/Blackwell support. |
| 2026-06-18 · **ml-engineering** (Stas Bekman) | Scale-up phase | Practical training-at-scale reference (throughput, multi-GPU, debugging). |
| 2026-06-18 · **bpeasy** (gautierdag) — fast BPE training | **Future Rust port only** | Can't swap tokenizers while distilling from Ouro (vocab must match — same constraint as T-FREE). Post-distill / from-scratch tool. **See "Tokenizer graduation" deep-dive — the controlled time to swap.** |
| 2026-06-18 · **Zett / FOCUS / WECHSEL** (zero-shot/heuristic tokenizer transfer) | **Tokenizer-graduation milestone** | Embedding+head re-init from sub-token pieces so the new vocab starts warm, not random; heal with short continued training. The mechanism for the graduation deep-dive below. |
| 2026-06-18 · DistiLLM-2 (Ko et al. 2024–25) — skew-KL + adaptive scheduling | When building Tier-2 on-policy | A refinement *of* the on-policy mode-seeking lever (divergence/scheduling). |
| 2026-06-18 · Neural text degeneration (Holtzman 2019) + contrastive decoding | Deployment / decode-time | Decode-side anti-repetition; we already have confidence/cycle stops. |
| 2026-06-18 · Deep Equilibrium Models stability (Bai et al. 2019) | Background | Theory for why our contractive recurrence (ρ(A)<1) converges to a fixed point — informs the noise/ρ levers. |
| 2026-06-18 · **Connected Papers** (connectedpapers.com) — citation-neighborhood discovery | Scouting | Seed from Huginn / GKD / MiniLLM / MoR to surface the recurrent-depth & distillation-efficiency clusters. |
| 2026-06-18 · **Looped-model scouting cluster** (to mine for transferable fragments) | Ongoing | **Mixture-of-Recursions (2025)** — recursion + routing for adaptive depth (closest to our ACT+MoE; top pick); **Relaxed Recursive Transformers** (Bae, 2410.20672) — recursion + LoRA-per-loop (we do this) + **Continuous Depth-wise Batching** (2–3× decode throughput; ties to parallel_loops saturation); **CoTFormer** — adaptive per-token depth; **Universal Transformers** (Dehghani 2018) + **PonderNet** (Banino 2021) — ACT/halt foundations; **Saunshi 2025** — looped depth-extrapolation theory; **ETD** (2510.07358) — loop reasoning-critical subset added at *mid-training*, +28% GSM8K @1B; **Loop-NN** (2409.14199) — basic looped refinement (superseded by the above). |
| 2026-06-18 · **MeSH — Memory-as-State-Highways for Recursive Transformers** (2510.07739, ICLR 2026) | **Collapse-relevant — deep-dive candidate** | Names our two failure modes: *uniform computation across iterations* (→ contractive collapse) and *single overloaded hidden state*. Fix: **external memory buffer + per-iteration routing for specialization**. Structural complement to the Huginn recipe (norm/LR/init). Matches bigger non-recursive @1.4B with 33% fewer params; code available. We have partial versions (loop-index embed + LoRA-per-loop). |
| 2026-06-18 · Throughput levers for distillation (the "speed up training" thread) | Now-ish | Linear-CE (head+CE fuse; helps hard-CE not distill-KL); MTP (backlog); seq-level KD (backlog); **sequence packing** — distill data appears already packed (P1.9 "packed distillation data" note) so likely DONE; num_workers dataloader A/B (queued). |
| 2026-06-21 · **SubQ-1.1-Small / Subquadratic Sparse Attention (SSA)** — Subquadratic AI (subq.ai), 2026 | **Far-future long-context phase** (post-coherence; the medical / whole-artifact RAG endgame) | Content-dependent **sparse attention with linear compute/memory** → 64.5× fewer attention FLOPs @1M ctx; trained @1M tokens, retrieval generalises to **12M** (RULER 99.12%). **Orthogonal to our recurrent core** — would swap attention (MLA→SSA), not the loop. Fit: reason over *whole* rare-disease literature / patient history in-context instead of chunking. **Tension to note:** SubQ's pitch is "stop chunk-and-retrieve, hold the artifact in context" — complement to, not replacement for, an external knowledge store (see RAG/continual-learning in roadmap; can't fit all PubMed in 12M tokens, and external = updatable/citable/safe, which medical needs). **⚠️ Competitive-use policy on subq.ai → REFERENCE ONLY** — read the report for ideas / benchmarks / recipe insights, but do NOT adopt their model/code/weights for a distributable product (same family as the OpenAI non-compete constraint we already avoid). **Implement the *technique* (subquadratic/sparse long-context attention) via clean-licensed alternatives, swapping MLA→sparse:** DeepSeek **NSA** (Native Sparse Attention, arXiv 2502.11789 — closest analog, fits our DeepSeek-aligned lineage; check for official code) · **MoBA** (Moonshot, github.com/MoonshotAI/MoBA — trainable block-sparse, *official open repo* = code in hand) · **Mamba/Mamba-2** (Apache 2.0 — true linear-time, but a bigger swap: SSM replacing attention). Verify each license at adoption. **Implementability (assessed 2026-06-21): a modular *swap*, NOT an architecture rewrite** — attention is already abstracted (`cfg.attn_type` = mla/gqa), so NSA/MoBA slot in as a third `attn_type` class with the recurrent spine / MoE / ACT / LTI untouched. *Real work* = integrating the sparse mechanism's KV/block-state with our recurrent **per-loop cache** (`recurrent_loop_*`) + sink + RoPE (contained to the attention layer). Mamba is more invasive (replaces QKV with an SSM sublayer). **Scale-gated:** ~zero benefit at our seq_len 1024 — only pays at long context, so it's a far-future feature that also implies enabling a long-context *regime* (data / seq_len / memory), not just the attention class. |
| 2026-06-20 · **MemPalace** (github.com/mempalace/mempalace, **MIT**) — local-first AI memory | **RAG / external-memory tier** (roadmap Stage A retrieval; post-coherence) | The off-the-shelf "memory palace": **verbatim** storage + semantic retrieval + temporal **entity-relationship knowledge graph**, structured wings/rooms/drawers; pluggable vector backends (Chroma/SQLite/Qdrant/pgvector); Python, mature. **Pairs with SSA** as complementary layers (external store → retrieve a slice → SSA reasons over the whole slice in-context — not competitors). Strong **medical** fit: local = patient privacy, KG = disease/symptom/treatment relations, verbatim+citable = never-hallucinate-from-weights. Candidate to plug in *instead of* building `RetrievalAugmentedInjector` from scratch. Check at adoption: embedding-model dependency + model-agnostic integration. |
| 2026-06-18 · DistiLLM-2 (contrastive / skew-KL) | When building Tier-2 on-policy | Already noted above — refinement of the on-policy mode-seeking lever. |
| 2026-06-22 · **VibeThinker-3B** (WeiboAI 2026 — arXiv 2606.16140; github WeiboAI/VibeThinker; hf WeiboAI/VibeThinker-3B) | **Validation now; levers for the future on-policy/RL stage** | Dense **3B** (Qwen2.5-Coder-3B base) hitting *frontier reasoning* (AIME26 94.3, IMO-AnswerBench 76.4%, LiveCodeBench 80.2) at **~$7.8k** post-training. **Core validation:** its "**Parametric Compression-Coverage Hypothesis**" = our exact bet — *verifiable reasoning compresses into a small param core; knowledge needs coverage* → independent proof a small model can be a frontier **reasoner**, knowledge left to retrieval (our RDT + MemPalace split; medical reasoning = compressible core, drug-facts/guidelines = retrieval). **Borrow (future):** (a) "**Spectrum-to-Signal**" diversity-then-amplify + **MaxEnt-Guided Policy Optimization (MGPO)** = entropy-preserving anti-mode-collapse — the *principle* speaks to our collapse fight, but it's **RL**, so it slots into the future on-policy/GKD stage, not current distillation; (b) **offline self-distillation** (sample → filter good trajectories → distill back) = a concrete instance of our parked on-policy lever. **Does NOT transfer wholesale:** dense **not RDT** (architecture not liftable — we reach small-reasoner via recurrence); **RL-heavy** (new capability, gated past coherence); **crucially the RL needs *verifiable* rewards** (math/code auto-checkable) — **medical largely isn't auto-verifiable**, so the core recipe fits only the *quantitative/rule-based* medical sub-tasks (dosing math, interaction lookups, guideline adherence), **not open diagnosis**. **Net: strategic validation + future-stage pointers, not a change-now recipe.** (Read from abstract/README/card; full MGPO/CLR mechanics need the paper.) |
| 2026-06-22 · **Nemotron-3-Ultra** (NVIDIA 2026 — research.nvidia.com/labs/nemotron/Nemotron-3-Ultra) | **Datasets to vet now; MOPD + Mamba-hybrid for later phases** | Frontier **550B / 55B-active** MoE **Hybrid Mamba-Attention** (+ MTP speculative-decode; 1M ctx, strong RULER@1M; 5.9× throughput vs GLM-5.1). Model itself **unusable** (hyperscaler scale; NVFP4 quant is Blackwell-native → *not* our Intel/NNCF INT4 path). **Useful pieces:** (a) **open datasets** — 173B GitHub code + synthetic factual/QA + Nemotron-Posttraining-v3 SFT/RL → candidate distill/SFT inputs; **vet via dataset_selection.md**, but *likely clean* (NVIDIA Open Model License permits output reuse — favorable vs the OpenAI-tainted MIRIAD we dropped); **none medical**; verify exact license at adoption. (b) **MOPD = Multi-teacher On-Policy Distillation** — validates our parked on-policy/exposure-bias cure, AND "multi-teacher" = the concrete mechanism to blend **Ouro (RDT signal) + a bigger dense teacher (capacity)** [ties to "distill from larger models once coherent"; vocab-match/converter per teacher still applies]. (c) **Mamba+attention+MoE hybrid** validates the SubQ/NSA/MoBA/Mamba long-context thread — refinement: **do a *hybrid* (Mamba + *some* attention), not pure-Mamba**. **Core caveat:** Nemotron is a standard *non-recurrent* stack → **silent on our recurrent-depth core**; the MoE/SSM/on-policy bits are orthogonal swap-ins, not recurrence guidance. (Read from the research page; layer ratio / token count not disclosed there.) |
| 2026-06-22 · **Self-Harness** (arXiv 2606.09498, 2026) — autonomous agent-harness self-improvement | **Deployment / product phase** (far-future; post-coherence + an agentic product to optimize) | LLM auto-improves its own *harness* (prompts / tools / agent-loop / scaffolding), **not the weights**: mine weaknesses from execution traces → propose minimal harness edits tied to failures → regression-validate, keep only safe ones (Terminal-Bench-2.0: MiniMax 40→62%, Qwen 24→38%, GLM 43→57%). **Fit:** if MythOuro ships **agentic** (retrieval/MemPalace → reason → cite → flag uncertainty), this auto-tunes that scaffolding to *our* model-specific failures — apt because (a) harness design is model-specific and a custom small medical model needs custom scaffolding, (b) could optimize **how the harness acts on the uncertainty head** (when to retrieve more / decline / flag) = the operational side of the honest-specialist thesis. **Caveats:** not model-related (wrapper only, no architecture/training bearing); applies only if the deployment is *agentic/tool-using* (little value for plain Q&A); it's *LLM-driven* → keep the optimizer local/open for sovereignty; check for released code vs re-implement. |
