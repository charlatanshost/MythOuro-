...
## 2026-09-26 (late) — 🔎 THE TEACHER CORPUS IS TWO DIFFERENT THINGS, AND ONLY ONE IS "TEACHER". Provenance, confabulation rate and token exposure, all four slices.

Completing the same-day corpus audit. The decisive artifact was the per-slice
`MANIFEST.json` — **two slices have one and two do not**, and that split explains
every quality difference measured today.

| slice | rows | MANIFEST | provenance | measured quality |
|---|---|---|---|---|
| `data_teacher_code` | 100,000 | ❌ none | **imported** OpenCodeInstruct | **96.4%** correct |
| `data_teacher_math` | 250,000 | ❌ none | **imported** nvidia/OpenMathInstruct-2 | **~65%** (n=20 hand) |
| `data_teacher_v2` | 12,109 | ✅ | **Ouro-2.6B generated** | see below |
| `data_teacher_med` | 5,023 | ✅ | **Ouro-2.6B generated** | see below |

**⇒ "teacher corpus" is a misnomer for 350,000 of its 367,132 rows.** The code and
math slices were never generated by our teacher; they are third-party datasets and
their quality is the dataset's, not Ouro's. Today's 96.4% and ~65% are judgements
about **OpenCodeInstruct and OpenMathInstruct-2**, not about Ouro. Only v2 and med
are actually teacher-written.

**MEASURED — the generation params for the two that ARE teacher-written** (identical, from both MANIFESTs):
`teacher_id ByteDance/Ouro-2.6B-Thinking · seed_len 48 · max_new 768 · temperature 0.9 · top_p 0.95`
`filters {min_new: 128, min_distinct1: 0.2, max_top_share: 0.5}`

- **94.1% of every row is invented** — 48 real seed tokens, up to 768 generated.
- **T=0.9** on factual prose. High-diversity sampling is the opposite of what
  factual reliability wants.
- **The filters are shape-only.** min length, a distinct-1 floor, a top-share
  ceiling. **Nothing checks whether any of it is true** — `gen_teacher_corpus.py`
  says so in its own docstring: *"Filters are deliberately dumb/fast… Spot-read
  before training on it."*

**Observed fabrication, medical slice.** A row reports a study with birth weight
"936 vs 946 g" and length "49.6 vs 50.4 cm". A 936 g infant is ~27 weeks and ~34 cm;
49.6 cm is a **term** length. The row is internally impossible — invented statistics
in house style, with sample sizes (n=500, n=501) and endpoint counts (27/38 vs 13/45)
to match. This is the mission domain.

**TOKEN EXPOSURE — how much of the stream each slice actually is** (teacher corpus
= 20% of the stream at `--teacher-data-ratio 0.2`; corpus ≈154M tokens):

| slice | tokens | of teacher corpus | **of the whole stream** | defective share of stream |
|---|---|---|---|---|
| math | ~78M | ~50% | **~10%** | **~3.5%** ← the prize |
| code | ~42M | ~27% | ~5.5% | ~0.2% |
| v2 | 8.65M | 5.6% | **1.12%** | 94% invented |
| med | 3.61M | 2.3% | **0.47%** | 94% invented |

**⚠ THE MEDICAL CONFABULATION WAS ALREADY KNOWN AND ALREADY MITIGATED — do not
re-raise it as an alarm.** `_MIX_RATIOS["medical"] = 0.20` was promoted from
harvest-only to the training mix on 2026-07-31 for exactly this reason, recorded
in `training_utils.py`: *"harvest-only meant the student had never read a real
abstract, only the teacher's confabulated imitations of them."* Real medical
(MedRAG/PubMed) is therefore **16% of the stream against 0.47% confabulated — about
34:1**. The mitigation works. What is new here is only the quantification.

**⇒ RANKED, BY TOKENS OF THE STREAM FIXED PER UNIT OF EFFORT — all CPU-only**
1. **Re-draw `data_teacher_math` from verified `problem_source`** (~3.5% of the
   stream currently teaching confident wrong reasoning). Biggest by an order of
   magnitude. ⚠ Feasibility unverified: OpenMathInstruct-2's 14M pairs are built
   from ~20k seed problems and most volume is augmented, so **check the
   non-augmented row count before planning on 250k**.
2. ~~Drop `data_teacher_v2`~~ / 3. ~~Drop `data_teacher_med`~~ — ❌ **RETRACTED
   SAME DAY, BEFORE ANY ACTION. Both recommendations were made without reading
   `docs/teacher_corpus_plan.md`, which states the reason these corpora exist.**

   **Why dropping them is wrong.** The harvest is the project's answer to a
   PROVENANCE problem, not a volume problem. Every SFT medical source failed
   provenance **4-for-4** (MIRIAD, Tulu-3, NuminaMath-CoT, ChemData700K), and
   MIRIAD — the core medical source — is **OpenAI-generated** (`clean_sft_datasets.md:57`:
   S2ORC "input to OpenAI's language models", plus an OpenAI-ToS ban on
   medical-diagnosis use). That is the exact constraint this project was rebuilt
   from scratch to satisfy. `teacher_corpus_plan.md:126` states the asymmetry
   plainly: *"instruction datasets are LLM-generated. The harvest seeds from raw
   human-written text and has Ouro write the continuations, so PubMed abstracts
   are clean by construction."* **Dropping `data_teacher_med` deletes the only
   provenance-clean medical route and re-opens the gap MIRIAD left.**

   **And the fabrication I reported was not a finding — it has been documented
   since 2026-07-31**, in `training_utils.py` above `"medical": 0.20`: the @70,500
   probe got *"the register perfect and the specifics invented — fabricated trials
   ('the PAWL study', n=300/n=306) with dosages that are simply wrong (30-mg daily
   ibuprofen, 15-mg daily naproxen; real dosing is 200-400mg and 220-550mg)"*, plus
   USMLE-style MCQ option-spam. My 936g/49.6cm row is the same phenomenon,
   re-observed. **Provenance-clean ≠ factually clean, and the project already knew
   it** — that is precisely why real abstracts were promoted into `_MIX_RATIOS`.

4. ❌ **ALSO RETRACTED, SAME DAY, BEFORE ANY ACTION — the distribution IS on
   purpose, and it is the most successful intervention in the project's history.**
   The owner asked whether the mix was deliberate before anything was changed. It
   is. `roadmap.md` item **D**: *"The constraint was token starvation in the teacher
   stream, and fixing it is **the only thing that has ever moved content**. The 20%
   teacher stream was 10.9M tokens — one epoch every 3,339 steps, re-read dozens of
   times by step 140,000. Two additions (code 35M, math 58M → 92M, one epoch every
   28,229 steps) took multi-statement solutions **3% → 15% → 28%** and L4 **5 → 14 →
   26 per 320**, monotone, while `body_stmts` had been median 1 at every α and every
   checkpoint ever measured."*

   **The pool grew 10x on purpose to stop the corpus being re-read, and it worked.**
   Medical's *share* fell as arithmetic; its *dose* did not — and I conflated the two.

   **Total medical exposure went UP, not down**, because `_MIX_RATIOS["medical"] =
   0.20` was added two days later:

   | | harvested | real abstracts | total | quality |
   |---|---|---|---|---|
   | 2026-07-29 | 5.89% | 0% | **5.89%** | 100% confabulated |
   | today | 0.55% | 16.0% | **16.55%** | **97% real** |

   **2.8x more medical, and almost all of it real PubMed text instead of invented
   trials.** There is no regression. Nothing needs re-weighting. The corrected
   reading of the dilution is that it is a *side effect of a deliberate, measured
   win*, fully compensated by a deliberate, measured fix.

   ⚠ **Lesson, and it is the third time today:** three separate recommendations
   (filter the code corpus, drop v2/med, restore medical weighting) were each
   produced by measuring something real and then reasoning about it without
   reading the doc that said why it was that way. The measurements were sound
   every time; the recommendations were wrong every time. **Grep the whole docs
   tree BEFORE forming the recommendation, not after the owner asks.**

5. **What the docs say to do instead — the documented, measured lever is to EXPAND
   the teacher corpus further,** and roadmap D lists what is still unused and
   provenance-clean: **OpenMathInstruct-2 ~14M pairs (250k used), OpenCodeInstruct
   200k (100k used), PubMedQA 100k (UNTOUCHED — the mission domain).** This is the
   known treatment for exactly the symptom `large` pt3 shows today (median
   `body_stmts` 1.0, 1% committed, 0.0% L4). **The one refinement today's
   measurement adds: when drawing the next tranche of math, prefer verified
   `problem_source` rows, because the unused 13.75M is mostly augmented and that is
   where the ~35% defect rate lives.** That is a refinement of plan D, not a
   substitute for it.

~~THE ACTUAL FINDING: MEDICAL HAS BEEN SILENTLY DILUTED 10.8x~~ — retracted above:

   `teacher_corpus_plan.md:152` records the 2026-07-29 decision: *"Combined pool
   12.26M → medical 29.4%, i.e. ~5.9% of training tokens at R=0.2 — chosen to make
   medical a co-equal fourth domain rather than dominate."*

   | | teacher pool | medical share | of all tokens |
   |---|---|---|---|
   | 2026-07-29, as designed | 12.26M | **29.4%** | **5.89%** |
   | today | ~132M | **2.7%** | **0.55%** |

   Nothing decided this. The `code` (100k rows) and `math` (250k rows) slices were
   imported later, and because `--teacher-data-ratio` feeds all four dirs as ONE
   `json:` glob with no per-corpus weighting, share is set by file size alone.
   Medical fell from co-equal fourth domain to 2.7% of the pool **as a side effect
   of adding other data**. For a project whose mission is medical, that is a
   regression against a written design decision, and it is invisible in every
   metric we track.

   ⚠ Before acting: `_MIX_RATIOS["medical"] = 0.20` (real MedRAG/pubmed abstracts,
   promoted 07-31) is untouched by this, so **16% of the stream is still real
   medical text**. The dilution is of the *harvested* slice only. The open question
   the 07-31 comment itself leaves — *"Revisit after the first probe that has seen
   real abstracts"* — has still not been answered.

**None of this needs the GPU.** It is data selection, and it can run during pours.

**⚠ NOT ESTABLISHED:** that any of it moves the student. Every structural A/B at
this budget has come back null (2026-09-24), and nothing here has been A/B'd. The
case for doing it is that the data is *wrong*, not that the fix is *measurable*.

## 2026-09-26 (night) — 🔴 THE MATH SLICE IS ~65% CORRECT, NOT 96%: the biggest corpus slice teaches confident wrong reasoning, and no cheap instrument can see it

Follow-on to the same-day corpus-quality measurement. The code slice came back
96.4% correct and the "improve the data" class was closed. **That conclusion does
not generalise.** `data_teacher_math` is **250,000 rows, 13 of 35 shards, 311MB —
the largest slice in the corpus** — and it is a different animal.

**THE TWO SLICES NEED DIFFERENT INSTRUMENTS, AND THAT IS THE FINDING**
- **code** rows *self-adjudicate*: the PROMPT carries `**Sample Input/Output**`,
  an independent check of the row's own answer. Mechanically gradeable.
- **math** rows *do not*: the only answer present is the teacher's own `\boxed{}`
  claim. There is nothing to check it against. The corpus cannot grade itself.

**MEASURED — hand adjudication, n=20 random rows (seeds 7 and 99)**

**13 clean / 7 defective ≈ 65% correct** (n=20, so the interval is wide — read it
as "far below the code slice", not as 65.0%).

The seven, and why:
| row | claimed | actual | failure |
|---|---|---|---|
| octagon area from midpoint quad | 800 | ~663 | ratio is √2/cos²22.5° = 1.657, not 2 |
| x³−6x²+15x+20 ≥ 0 | `[-1,4]∪[5,∞)` | ≈`[-0.95,∞)` | p′=3(x²−4x+5) has negative discriminant → p is strictly increasing, so the set is ONE ray |
| distance (3,4) to y=x²+2x−1 | 2√2 ≈ 2.83 | ≤1.52 | x=1.5 already gives 1.52 |
| count a with f(a)g(a) square | 499 | fewer | asserts (X−Y)(X+Y)=(X−Y)²; at a=3, f·g=577, not square |
| sum of REAL roots of x³−5x²+11x−7 | 5 | 1 | Vieta gives ALL roots; (x−1)(x²−4x+7), discriminant −12 |
| incircle-centre quadrilateral | 200 | — | problem is incoherent (C cannot vary), derivation never reaches 200 |
| shirt combinations | 720 | — | problem is unanswerable (rules never given); the model SAYS SO, then boxes 720 anyway |

**Every simple arithmetic word problem in the sample was correct.** The failures
are entirely in the reasoning-dense problems — geometry, number theory,
optimisation, root analysis — plus two rows whose PROBLEM is defective.

**⚠ THE MECHANICAL CHECK IS NEARLY BLIND. Do not quote it as the error rate.**
`tools/corpus_math_check.py --arith` verifies explicit numeric claims and flags
**4.16% of assertions / 5.57% of checkable rows**, and only **11.2% of rows expose
a checkable assertion at all**. Hand-inspection of 20 flags found **~40% real**
(`47*93=4361`→4371; `9118+33182=42200`→42300; `(1.04)³=1.125984`→1.124864;
`999 = 8*112+95`→991 inside a Euclidean algorithm) and ~60% artifacts (LaTeX `\div`
rendered `:`, stripped variables turning `14x = -26` into `14 = -26`, factorials,
legitimate rounding). So the mechanical rate is ~2% against a hand-measured ~35%.
**It misses ~94% of the defects, because the defects are reasoning with perfect
arithmetic.**

Canonical demonstration, shard_0000 row 1 — "sum of all three-digit integers whose
cubes end in 125", answered `125+375+625+875 = 2000`. Every addition is correct.
But 375³ ends 375, 625³ ends 625, 875³ ends 875: **three of the four do not satisfy
the condition**, 18 valid values are missing, and the answer is **11990**. An
arithmetic checker passes this row.

**⇒ HYPOTHESIS, NOT A CONCLUSION — but the shape is a suspicious match.** The
student produces *well-formed, confident, wrong* output: wide pt3 is 68.2% L3+ /
42.4% committed / **4.5% L4**, and large pt3 is 52% correctly-indented / 1%
returning a value / **0.0% L4**. The largest slice of its training corpus is
well-formed, confident, and wrong about a third of the time on exactly the
reasoning-heavy examples. That may be cause, or the student may simply be unable
to do the reasoning regardless. **It is not established, and it must not be
written up as if it were.**

**This partially REOPENS the data-quality lever that the code measurement closed.**
Closed for code (nothing to filter). Open for math. Unlike code, math answers
cannot be filtered by execution — but they can be filtered by **self-consistency**:
sample the teacher k times per problem, keep rows whose `\boxed{}` answers agree.
That is GPU work and competes with the pour, so it is NOT scheduled here.

**Still unmeasured:** `data_teacher_v2` (12,109 rows: math 39%, general 43%, code
18%) and `data_teacher_med` (5,023 rows) — and medical has no ground truth at all,
which for the mission domain is the harder problem.

**Reproduce:**
`python3 tools/corpus_math_check.py --review 10 --seed 7` (hand adjudication — the only trustworthy method)
`python3 tools/corpus_math_check.py --arith -n 4000` (lower bound, read the warnings)

## 2026-09-26 (evening) — ⏹ CORPUS QUALITY IS NOT THE CONSTRAINT: the code slice is 96.4% correct and 100% parseable. "Better data" is closed.

Asked by the owner: *"since we are distilling, wouldn't it take less tokens
because we are using tokens of much better quality?"* The right way to answer was
to measure the quality instead of assuming it. New tool: `tools/corpus_correctness.py`
(CPU-only, ran alongside a live training leg).

**⚠ CORRECTION, made in the same conversation.** Earlier that day I claimed the
teacher is "70.6% L4, so roughly a third of what it produces on code is wrong,
and we distil toward its wrong answers." **That was wrong.** 70.6% is the teacher's
score under the PROBE's adversarial settings — bare `def f(x):` framing, a
96-token budget, T=0.4. The corpus is full chat-formatted generations with a
think block and a complete answer. Different instrument, different number. The
inference from one to the other was unwarranted and the filtering idea built on
it does not survive.

**MEASURED — `data_teacher_code`, 100,000 rows, n=3,000 sampled, seed 1**

| | rows | rate |
|---|---|---|
| contain a ```python block | 3000 | **100.0%** |
| block parses (`ast.parse`) | 3000 | **100.0%** |
| self-adjudicating (1 top-level fn, 1 arg, literal sample I/O) | 840 | 28.0% |
| **correct, strict `==`** | 766 | **91.4%** |
| **correct, type-normalised** | 808 | **96.4%** |
| residue | 30 | 3.6% |

Strict `==` understates: it counts `(1,2)` vs `[1,2]` and `'101010'` vs `101010`
as failures. **Read 96.4%.**

**READ THE RESIDUE — it is dominated by NON-defects.** Hand-inspected, the 30
survivors are mostly:
- functions that sort/reverse **in place** and return `None` (`bubble_sort`,
  `merge_sort`, `reverse_list_in_place`) — the work is done, the contract differs
- **`random`-based** functions whose sample output cannot be reproduced by
  construction (`generate_random_integers`, `dice_game`)
- set/powerset **ordering** differences (`generate_combinations`)
- rows where the **SAMPLE OUTPUT ITSELF IS WRONG and the code is right**:
  `calculate_total_sales` — 120.5 + 199.75 = **320.25**, the prompt asserts 310.25;
  `count_unique_elements` — `[1,2,2,3,-1,-1,4]` has **5** unique values, the prompt
  asserts 4.

Clear cases of the teacher writing wrong code: **close to none** in the sample
inspected. The true rate is at or above 96.4%, not below it.

**⇒ THE "IMPROVE THE DATA" CLASS IS CLOSED for the code slice.** Correctness-
filtering has essentially nothing to remove. Whatever is stopping the student, it
is not that it is being taught wrong answers.

**What this does to the token-efficiency question.** The tokens really are high
quality — 100% syntactically valid, ~96% correct, fully soft-target supervised.
We poured **2.85B** of them into wide pt3 and got **4.5% L4** while reaching
**68.2% L3+** and **42.4% committed** (above SmolLM2-360M's 37.8% / 32.2% at ~4T
tokens). So distillation IS buying enormous token efficiency — on FORM. It is not
buying correctness, and the reason is now measured not to be signal quality.
Same shape as large pt3 the same day: 52% of completions learned where the indent
goes, 1% learned to return a value.

**Caveats, stated because they bound the claim.**
- Only **28% of rows self-adjudicate**, and they skew simple — one argument,
  literal sample I/O. 96.4% is an **upper bound** for the whole corpus.
- Only the **code** slice is measured. `data_teacher_math`, `_med` and `_v2`
  (311MB / 35MB / 76MB) are unmeasured; math has checkable final answers and is
  the obvious next target if the question comes up again.
- The 3.6% residue was inspected by hand, not classified by the tool. The tool
  prints it for exactly that reason — do not quote it as a defect rate.

**Reproduce:** `python3 tools/corpus_correctness.py --glob 'data_teacher_code/shard_*.jsonl' -n 3000 --json reports/corpus_correctness_code.json`

## 2026-09-26 (later) — ✅ TAKE THE 1.4B TEACHER: its ceiling is 62.8% L4, the student is at 0.0%, and it pours 1.38x faster

The teacher-gap question finally has a measurable answer, because it was asked on
the right instrument. The 09-24 A/B compared **soft-KL against different
teachers** — not comparable, correctly thrown out. This compares both teachers on
the **student's own code ladder**, which is teacher-independent by construction.

**MEASURED — the two ceilings (`code_eval --hf-model`, n=320, T=0.4 pen=1.15 seed=1234, bare, max_new 96)**

| model | n | L0 | L3+ | L4 | committed | median body_stmts |
|---|---|---|---|---|---|---|
| Ouro-2.6B-Thinking | 320 | 0.6% | 97.8% | **70.6%** | 72.8% | 1.0 |
| Ouro-1.4B-Thinking | 320 | **0.0%** | 94.7% | **62.8%** | **76.2%** | 1.0 |
| student large pt3 (139M tok) | 960 | 71.5% | 16.8% | **0.0%** | 0.2% | 1.0 |
| student wide pt3 (2.85B tok) | 960 | 7.7% | 68.2% | 4.5% | — | — |

The 1.4B is **7.8 pp lower on L4** (relative −11%) and **better on the two axes the
student is furthest behind on**: committed 76.2% vs 72.8%, L0 0.0% vs 0.6%.

**MEASURED — throughput, same variant, same 921,907,763-param student**
- 2.6B teacher: **14.98 s/step**, 1.107k tok/s (`curve_large_leg2_20260925_0108`, 4,200 steps)
- 1.4B teacher: **10.81 s/step**, 1.525k tok/s (`curve_large_t14_leg1_20260924_0550`, steps 10→1800, 5.37h)
- **1.386× by step time, 1.378× by tok/s** — two independent measures, consistent.
- In a 17.5h window: **95.4M tokens vs 68.8M**. Tokens/step is 16,384 either way;
  what changes is tokens per HOUR.

**⏸ HOLD — THIS DECISION IS SUSPENDED (2026-09-26 late). THE THROUGHPUT PREMISE
COLLAPSED.** The swap's entire justification was 1.38x. Measured today, the **2.6B
teacher is running at 9.88 s/step** — *faster than the 1.4B has ever been measured*:

| leg | teacher | s/step | start | mean `depth` | mean `unc` |
|---|---|---|---|---|---|
| leg1 | 2.6B | 14.98 | Mon 21 | 0.0439 | 0.1260 |
| leg2 | 2.6B | 15.07 | Tue 22 | 0.0144 | 0.1298 |
| bal_leg1 | 2.6B | 14.92 | Wed 23 | 0.0739 | 0.1238 |
| bal_leg1 | 2.6B | 15.07 | Wed 23 | 0.0170 | 0.1404 |
| **t14_leg1** | **1.4B** | **10.81** | Thu 24 | 0.0443 | 0.1266 |
| leg1 | 2.6B | 14.97 | Thu 24 | 0.0291 | 0.1196 |
| leg2 | 2.6B | 14.95 | Fri 25 | 0.0165 | 0.0597 |
| **leg3** | **2.6B** | **9.88** | **Sat 26** | **0.0037** | **0.0328** |

Six 2.6B legs sat in **14.92–15.07** — a 1% band across five days. Today's is 9.88.
**Ruled out:** config drift (headers byte-identical: same variant, 921,907,763
params, same teacher, same 35 shards); thermal (both legs are internally flat to
±1% across 17h); the two inference servers (`ouro-server`, `open-webui` — **0
render-node fds**, `xpu-smi` shows only the trainer on the card); `op` accumulation
(5.65/8 today vs 5.69–5.74 before).

**Strongest correlate, NOT PROVEN: `depth` collapsed to 0.0037** (4–20x below every
prior leg) and `unc` to 0.0328. With `n_loops 4` as a *maximum* and ACT deciding the
realized depth, a model that has finally started halting early computes fewer loops
per token and gets faster. That is the benign reading and it fits the exit_pdf
objective's whole purpose — but it is a correlation across 8 legs, not a measurement.

**⇒ DO NOT SWAP THE TEACHER.** The 1.4B was measured Thu 24 at `depth 0.0443` — the
old regime. Comparing it to today is comparing across whatever changed. Right now
the 2.6B is **both faster and higher-ceilinged** (70.6% vs 62.8% L4), so there is no
case for the swap at all. Revisit only if s/step returns to ~15, and then
**re-measure the 1.4B under current conditions before deciding** — the original
1.38x is not a usable number any more.

~~DECIDED: switch at the next leg boundary (step 12,600).~~ The 1.4B's ceiling is
**14x above the best student this project has ever produced** (4.5% L4) and
unboundedly above the current one (0.0%). The 7.8 pp between the two ceilings
cannot bind until the student is somewhere north of 60% L4, which is not on this
year's trajectory. Paying 28% of every remaining night to preserve headroom the
student cannot reach for years is the wrong trade when **tokens are the measured
constraint** (2026-09-22).

**The honest cost.** Swapping the teacher changes the soft-KL target, so the loss
series is **not comparable across step 12,600**. The probe instruments are — fixed
prompts, fixed decode, fixed grader, no teacher in the loop — so the token curve,
which is the actual instrument, survives the boundary. Loss was never the
instrument here.

**What we still cannot measure, and are not pretending to.** Whether the 1.4B
*teaches* as well is unresolved. `checkpoints_large_t14/` holds a 1,800-step
1.4B-taught arm that was never read out on the probes — but its matched 2.6B
control (large pt1, same 1,665/1,746/1,800 steps) sits at **98.9% L0**, i.e. near
total degeneracy, so the ladder has almost no dynamic range there. That is the
09-24 rule restated: **no instrument separates arms at 29M tokens.** Spending legs
to buy an attribution we have proven we cannot make is the more expensive error.

**Protocol for the switch**
- First switched leg at **LEG=4200** (12.6h at 10.81 s/step), not a longer one:
  short enough to catch damage cheaply, and **step_0012600.pt is a backbone
  checkpoint** (12600 % 4200 == 0) so the prune retains it and a revert costs one leg.
- **Watch the first ~100 steps.** A target shift shows as a loss jump that does not
  recover within a few hundred steps. Both models share the vocab and lineage so
  the distributions should be close, but "should" is not measured.
- Once one switched leg is clean, step up to **LEG=6300** (18.9h, MILE=2100,
  12600 % 6300 == 0) to actually bank the throughput on a full window. Do not use
  LEG=5040/5400 — they do not divide 12,600 and silently produce a stub leg.

## 2026-09-26 — 🟢 LARGE, POINT 3 (8,400 steps / ~139M tok): THE FIRST NON-FLAT POINT IN THIS LINEAGE — and L3+ has become an INDENTATION detector

First point on any lineage since 2026-09-14 that moved on more than one axis at
once. It is real and it is broad — and the rung that moved is not measuring what
its name says. Both halves matter; do not quote the first without the second.

**Run (MEASURED, from `logs/curve_large_leg2_20260925_0108.log.err`)**
- resumed `checkpoints_large/step_0004200.pt` → **8,400/8,400**, 01:08:13 → 18:36:34 = **17h 28m**
- **14.98 s/step** over 4,200 steps; mean **1.107k tok/s** over 420 log lines
- this leg ≈ **69.6M tokens** (16.6k tok/step); **cumulative ≈ 139M** at step 8,400
- loss by quarter **1.625 → 1.427 → 1.360 → 1.301** (first10 1.755 → last10 1.240).
  Leg 1 was 4.019 → 2.184, leg 2 2.143 → 1.907. **Still absorbing. No plateau.**
- ⚠ the log's `leg2` label is the string passed to `run_token_curve.sh`, not a leg
  counter: this run is the **third** leg and its milestones are filed as `leg3`.
  The `leg1_20260924_1257` log is 3,600→4,200 (600 steps). Join by STEP, not label.

**The curve (n=960 code across 3 milestones; prose n=90)**

| point | Mtok | top_share | distinct1 | halt | LOOP | L0 | L2+ | L3+ | L4 |
|---|---|---|---|---|---|---|---|---|---|
| large pt1 | 29 | 0.206 | 0.411 | 2.16 | 0/90 | 98.9% | 1.1% | 0.3% | 0.0% |
| large pt2 | 59 | 0.181 | 0.426 | 2.08 | 0/90 | 98.1% | 1.9% | 0.4% | 0.0% |
| **large pt3** | **139** | **0.155** | **0.464** | 2.06 | 1/90 | **71.5%** | **28.5%** | **16.8%** | **0.0%** |
| 240M @2.85B (ref) | 2,850 | 0.098 | 0.559 | 2.75 | 4/90 | 7.7% | — | 68.2% | 4.5% |

Every prose axis moves monotonically toward the reference across all three points.
`halt` is the exception: **flat at 2.06–2.16** against the reference's 2.75.

**L3+ went 0.4% → 16.8%, and it is INDENTATION (MEASURED, not inferred)**
- **completions starting with an indent: 1.5% → 52.2%.** That is the whole delta.
- of 161 L3 samples, **2 return a value** (`committed`) — **1%**. At point 2 it was 1/4.
- median `body_stmts` = **1.0 at both points**. Median `lines_dropped` 1 → 2.
- mechanism, from `tools/code_eval.py:386-433`: the PROMPT supplies `def fname(...):`,
  `_truncate_to_parseable` keeps the longest parseable prefix, so
  `def f(n):\n\t<one statement that doesn't raise>` parses (L1), defines (L2), runs,
  returns `None`, fails the assert → **L3**. No computation is required to score L3.
- it is **not** a one-task artifact: rung≥3 rose on **all 10 tasks**
  (is_prime 30.2%, double_it 27.1%, is_even 22.9%, reverse_string 21.9%,
  fibonacci 17.7%, add_two/get_first/max_of_two 12.5%, count_items 6.2%, sum_list 4.2%).
  At point 2 only 4 samples in 960 cleared it, spread over 4 tasks.
- the text confirms it: rung-3 bodies are `h_0 = 1`, `n = 1`, `n_1 = 0` followed by
  English prose about the problem. It learned where the indent goes, not what to put there.

**Reading (INFERRED)**
What arrived at ~139M tokens is **Python block structure**, the surface container —
exactly the acquisition order recorded 2026-08-06 ("container before content"). The
model now knows a `:` is followed by an indented line; it does not yet know what the
line should compute. L4 is still 0/960, and `committed` is still ~0.

**⇒ INSTRUMENT RULE for this lineage: L3+ is no longer a capability metric.**
It saturates on indentation and will drift toward ~100% while correctness stays at
zero. **Track `committed` and L4 as the live rungs from here**, plus L0 for degeneracy.
A rise from 16.8% → 40% next leg is NOT capability and must not be written up as such.
This is the same failure mode as the 2026-07-30 code series (the attractor hid
structure, not correctness) with the polarity reversed: now structure hides absence.

**What this does NOT resolve.** `halt` has not moved in 139M tokens (2.06 vs the
reference's 2.75), so ACT is not yet using depth on this lineage. And one LOOP case
appeared at point 3 (0 → 1/90) — inside noise at n=90, but watch it, not average it.

**Action:** keep pouring. `LEG=4200` on weekdays, `LEG=1800` short nights, readout
every third leg. Nothing here argues for a structural change, and per 2026-09-24 no
structural A/B is measurable at this budget anyway.
...
The first time this lineage's LR went below 3e-5. Completed cleanly; the stop
rule did not fire (LOOPING 4/90, no salad).

| | wide pt3 | **anneal** | | per-ckpt L3+ |
|---|---|---|---|---|
| distinct1 | 0.559 | 0.546 | flat/down | |
| halt | 2.75 | 2.71 | flat | |
| top_share | 0.098 | 0.104 | flat/down | |
| L0 | 7.7% | **4.0%** | down (good) | |
| **L3+** | 68.2% | **81.7%** | **+13.5pp** | 69/59/76 → **77/88/81** |
| **L4** | 4.5% | 3.6% | **flat** | 2.8/9.4/1.2 → 0.0/3.1/7.8 |

Three-checkpoint means on both sides. The anneal's worst code checkpoint
(77%) beats wide pt3's best (76%) — a real separation on L3+.

### And it is exactly the 08-21 pattern

*"The α-anneal moved PARSEABILITY, not correctness. L3+ is not a capability
metric."* Same shape: L3+ up hard, L4 flat, prose flat. Reading the code:
`fibonacci` opens `if n <= 1: return 0` and then emits an expression that
runs and means nothing. The lower LR settled the model into producing more
code that *executes* and no more code that is *right*. The gate for this leg
was pre-registered on L4 and distinct1 precisely because of 08-21, and on
those it is flat.

Two single-checkpoint L4 moves at step 3,000 (`reverse_string` 0 → 10/32,
`is_even` 0 → 7/32) did not appear at 1,000 or 2,000 — the 3-ckpt L4 mean is
3.6%. Recorded, not read.

Training log across the anneal: loss 0.585 → 0.603, soft-KL 0.739 → 0.756
(first third → last third). An anneal that lands usually settles loss down;
this drifted up. Small, and consistent with "nothing to converge to here".

### What is worth keeping

L0 4.0% and L3+ 81.7% are the best three-checkpoint code numbers the project
has, and for a coding assistant "more of the output is runnable" is not
nothing. The annealed checkpoint at 1e-5 is trainable. But it is not the
capability the ceiling measurement is about.

### ⇒ By elimination

Tokens, FFN width, α (fully annealed), the LR floor, and the teacher are now
all measured and none of them is what holds L4 at ~4% and distinct1 at ~0.55
while the teacher sits at 71% and 0.615. Soft-KL keeps falling; the probes do
not move. The roadmap wrote the conclusion in August, rung 6: *"more offline
tokens — continued OR fresh — only sharpen the attractor. The cure is a
different objective, not more data."* That is now the result of every cheap
test rather than a hypothesis. Rung 6 — on-policy SFT, generate from the
student and correct against the target — is the direction. Multi-session
build; the roadmap called it the front-runner for after the pour, and the
pour has reported.
